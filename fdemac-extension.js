(() => {
  ...
  branch.innerHTML = `
    ...
    <div class="fdex-card fdex-track-card"><div class="fdex-head"><div><h4>呼朋引猹</h4><span class="fdex-complete-label">已完成</span><strong class="fdex-complete-count"><span data-invites>0</span>/30</strong></div><button class="fdex-text-btn" data-simulate>演示邀请 +1</button></div><div class="fdex-steps">${[[3,50],[10,100],[30,300]].map(([n,reward])=>`<div class="fdex-step" data-step="${n}"><div class="fdex-node"><span><img src="./assets/guazi.png" alt="瓜子">${reward}</span></div><b>邀请 ${n} 位新猹友</b><span class="fdex-pill">未解锁</span></div>`).join('')}</div></div>
    ...
    <div class="fdex-card fdex-rank-card"><div class="fdex-head"><div><h4>一呼百应</h4><p>用影响力说话，邀请更多好友加入共学，赢取排行榜好礼。</p></div></div><div class="fdex-prizes"><div class="fdex-subhead"><b>排行榜奖品</b></div><div class="fdex-podium-wrap"><div class="fdex-podium">${[rows[1],rows[0],rows[2]].map(([r,a,n,c],index)=>{const top=index===0?2:index===1?1:3;const prize=top===1?'Mac mini':top===2?'键盘 / 耳机':'精选数码好礼';return `<article class="fdex-podium-card top${top}"><small>TOP ${top}</small><img class="fdex-podium-avatar" src="./assets/avatars/${a}.svg" alt="${n}的头像"><b>${n}</b><span>${c} 人 · ${prize}</span></article>`}).join('')}</div></div><div class="fdex-follow-prize"><b>TOP 4–10</b><span>TokenDance 白金卡</span></div></div><div class="fdex-board"><div class="fdex-subhead"><b>实时排名</b><button class="fdex-text-btn" data-refresh>刷新排名</button></div>${rows.map(([r,a,n,c])=>`<div class="fdex-row"><span class="fdex-rank">${r}</span><span class="fdex-person"><img class="fdex-avatar" src="./assets/avatars/${a}.svg" alt="${n}的头像"><b>${n}</b></span><span class="fdex-count">${c} 人</span></div>`).join('')}<div class="fdex-me"><span>你的排名：<strong data-rank>尚未入榜</strong></span><span data-board-gap>再邀请 15 人即可入榜</span></div></div><details class="fdex-rules" data-rules-box><summary>活动规则说明（含两种拉新形式）</summary><div class="fdex-rule-grid"><p><b>呼朋引猹</b>好友通过你的专属链接完成活动报名，即为有效邀请。阶梯奖励累计解锁，瓜子将在达标后自动发放。</p><p><b>一呼百应</b>排行榜按有效邀请人数实时更新；每位用户仅计入一位邀请人，最终名次及获奖资格以活动结束后的平台核验结果为准。</p></div></details></div>`;
  ...
  branch.querySelector('[data-simulate]').onclick=()=>{invites=invites<3?3:invites<10?10:30;update();say('已切换到下一档演示状态')};
