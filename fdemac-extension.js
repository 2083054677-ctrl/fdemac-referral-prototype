(() => {
  ...
  let invites = demoParams.has('fdexInvites')?Math.max(0,Number(demoParams.get('fdexInvites'))||0):2;
  let viewerRank = demoParams.has('fdexRank')?Math.max(0,Number(demoParams.get('fdexRank'))||0):165;
  ...
  const update=()=>{document.querySelectorAll('[data-invites]').forEach(el=>el.textContent=invites);const track=branch.querySelector('[data-progress-track]');track.style.setProperty('--progress',`${Math.min(Math.max(invites,0),30)/30*100}%`);branch.querySelector('[data-current-invites]').textContent=`已邀请 ${invites} 人`;const me=branch.querySelector('[data-me]');me.hidden=invites<=0||!Number.isInteger(viewerRank)||viewerRank<=0;if(!me.hidden){branch.querySelector('[data-rank]').textContent=`第 ${viewerRank} 名`;branch.querySelector('[data-my-invites]').textContent=invites}branch.querySelectorAll('[data-step]').forEach(el=>el.classList.toggle('done',invites>=+el.dataset.step))};
  window.__setFdexViewerRanking=({invites:nextInvites,rank:nextRank})=>{invites=Math.max(0,Number(nextInvites)||0);viewerRank=Number.isInteger(Number(nextRank))?Number(nextRank):0;update()};
