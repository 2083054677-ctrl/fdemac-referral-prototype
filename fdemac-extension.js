(() => {
  ...
  branch.innerHTML = `
    ...
    <div class="fdex-card fdex-track-card"><div class="fdex-head"><div><h4>呼朋引猹</h4><span class="fdex-complete-label">已完成</span><strong class="fdex-complete-count"><span data-invites>0</span>/30</strong><p class="fdex-track-desc">邀请好友报名共学营，让集体学习的氛围助力彼此坚持与成长。</p></div><button class="fdex-text-btn" data-simulate>演示邀请 +1</button></div><div class="fdex-steps">${[[3,50],[10,100],[30,300]].map(([n,reward])=>`<div class="fdex-step" data-step="${n}"><div class="fdex-node"><span><img src="./assets/guazi.png" alt="瓜子">${reward}</span></div><b>邀请 ${n} 位新猹友</b><span class="fdex-pill">未解锁</span></div>`).join('')}</div></div>
